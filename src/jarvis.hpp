...
private:
    ...
    std::string api_key = "657fae88735f4b169340b3ba95b6e198";
...
private:
    ...
    std::string app_id="16704023";
    //你注册完毕百度语音识别平台的账号后，在平台上就会有你的api_key
    std::string api_key="Xvnm2IyHbQY82D8Y7EczEPam";
    //你注册完毕百度语音识别平台的账号后，在平台上就会有你的secret_key
    std::string secret_key="VglhFyYydE66ZEwr2y5uqZtgWy9vLwth";
    //SDK对应的语音识别客户端
    aip::Speech *client;
public:
    SpeechRec()
...
public:
    ...
    void run()
    {
        ...
        while(!quit){
            ...
            if(ret){
                std::string cmd;
                std::cout << "Lucifer: " << message << std::endl;
